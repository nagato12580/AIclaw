...
import { a2aApi, type A2ARemoteAgent, type A2ATask, type A2ASubscribeEvent, extractTextFromParts } from "@/api/a2a";
...
export function ChatInterface() {
  ...
  const applyA2aSubscribeEvent = (sessionKey: string, taskId: string, event: A2ASubscribeEvent) => {
    ...
    if (event.type === "TaskArtifactUpdateEvent") {
      ...
      const content = artifact?.parts && artifact.parts.length > 0
  ...
  const runA2aMessageFlow = async (sessionKey: string, assistantId: string, inputText: string) => {
    ...
    const payload = {
      ...
      message: { role: "user" as const, parts: [{ kind: "text" as const, text: inputText }] },
