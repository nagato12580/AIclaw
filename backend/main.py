...
async def nanobot_chat(request: ChatRequest):
    ...
    3. 设置请求上下文（供后续工具调用）
    4. 根据路由模式注入运行时指令
    5. 调用 nanobot_service 处理消息
    6. 提取 artifacts 和可视化数据
    7. 持久化会话增强信息
    8. 组装并返回响应
    
    Args:
        request: ChatRequest 对象，包含会话ID、消息内容、路由模式、模型ID等
    ...
    Returns:
        dict: 响应字典，包含以下字段：
            ...
            - viz: 可视化数据（可选）
            - routing: 路由信息
            - artifacts: 提取的 artifacts（可选）
            - usage: Token 使用量（可选）
            - kb_citations: 知识库引用（可选）
            
    Raises:
        HTTPException: 处理过程中发生异常时返回 500 错误
    """
    try:
        # 同步会话与项目的关联关系
        ...
        if request.route_mode == "sql" or request.prefer_sql_chart:
            ...
            # 纯聊天模式：禁止使用 nl2sql 工具
            instructions.append("Normal chat mode. Do NOT use the nl2sql tool")

        # 将指令注入到消息中（作为 Runtime Context）
        if instructions:
            instr_block = "\n".join(instructions)
            # 如果消息已有 Runtime Context，追加指令；否则创建新的
        ...
        # 持久化会话增强信息（可视化、artifacts、usage 等）
