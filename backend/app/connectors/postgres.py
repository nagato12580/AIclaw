...
class PostgresConnector:
    ...
    # Metrics
    POSTGRES_QUERY_COUNTER = Counter("postgres_queries_total", "Total Postgres queries", ["status"])
    POSTGRES_QUERY_LATENCY = Histogram("postgres_query_latency_seconds", "Postgres query latency seconds")
    ...
    def execute_query(self, query: str):
        ...
        try:
            ...
            POSTGRES_QUERY_COUNTER.labels(status="success").inc()
            return rows
        except Exception as e:
            POSTGRES_QUERY_COUNTER.labels(status="error").inc()
            raise
        finally:
            POSTGRES_QUERY_LATENCY.observe(time.perf_counter() - start)
