...
def _get_cached_connector(ds_type: str, config_json: str):
    ...
    if ds_type in ["postgres", "postgresql", "supabase"]:
        ...
        if not db_url:
            ...
            db_url = f"mssql+pymssql://{config.get('user')}:{config.get('password')}@{config.get('host')}:{port}/{config.get('database')}"
        return PostgresConnector(db_url=db_url)

    elif ds_type in ["oracle"]:
        db_url = config.get("connection_string")
        if not db_url:
            port = config.get("port") or 1521
            db_url = f"oracle+oracledb://{config.get('user')}:{config.get('password')}@{config.get('host')}:{port}/{config.get('database')}"
        return PostgresConnector(db_url=db_url)
