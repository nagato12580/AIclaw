...
# Prometheus metrics
NL2SQL_REQUESTS = Counter(
    "nl2sql_requests_total", "Total NL2SQL requests", ["source", "model"]
)
NL2SQL_LLM_LATENCY = Histogram(
    "nl2sql_llm_latency_seconds", "LLM generation latency seconds", ["model"]
)
NL2SQL_SQL_LATENCY = Histogram(
    "nl2sql_sql_latency_seconds", "SQL execution latency seconds", ["source"]
)
NL2SQL_CACHE_HITS = Counter("nl2sql_cache_hits_total", "NL2SQL cache hits", ["cache_type"])
NL2SQL_ACTIVE = Gauge("nl2sql_active_requests", "Active NL2SQL requests")
...
def _get_upload_payload(file_url: Optional[str]) -> Dict[str, Any]:
    ...
    with _cache_lock:
        ...
        if cached and now < cached["expires_at"]:
            try:
                NL2SQL_CACHE_HITS.labels(cache_type="upload").inc()
            except Exception:
                pass
...
async def process_nl2sql(
    ...
    with _track_active_request():
        try:
            NL2SQL_REQUESTS.labels(source=request.source, model=(get_active_llm_config() or {}).get("model", "unknown")).inc()
        except Exception:
            pass
        ...
        if connector:
            ...
            if cached_schema is not None:
                try:
                    NL2SQL_CACHE_HITS.labels(cache_type="schema").inc()
                except Exception:
                    pass
            ...
            for attempt in range(NL2SQL_LLM_RETRY_COUNT + 1):
                try:
                    with trace_service.start_span(
                        ...
                        # measure LLM latency
                        with NL2SQL_LLM_LATENCY.labels(model=active_config.get("model")).time():
            ...
            with trace_service.start_span(
                ...
                if request.source == "upload":
                    if upload_df is None:
                        upload_payload = await asyncio.to_thread(_get_upload_payload, request.file_url)
                        upload_df = upload_payload["df"]
                    with NL2SQL_SQL_LATENCY.labels(source="upload").time():
                    ...
                    with NL2SQL_SQL_LATENCY.labels(source=request.source).time():
