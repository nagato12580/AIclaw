...
class A2APartCreateSchema(BaseModel):
    part_type: Optional[A2APartType] = None
    ...
    def _normalize_part_type(self) -> "A2APartCreateSchema":
        if self.part_type is None and self.kind:
            self.part_type = A2APartType(self.kind)
        if self.part_type is None:
            self.part_type = A2APartType.TEXT
        return self
...
class A2AMessageCreateSchema(BaseModel):
    ...
    @model_validator(mode="after")
    def _ensure_message_id(self) -> "A2AMessageCreateSchema":
