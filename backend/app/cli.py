...
def _ready_probe_host(host: str) -> str:
    if host in {"0.0.0.0", "::"}:
        return "127.0.0.1"
    return host
...
def _wait_for_server_ready(host: str, port: int, timeout: float) -> bool:
    probe_host = _ready_probe_host(host)
...
def _is_port_accepting(host: str, port: int) -> bool:
    try:
        with socket.create_connection((_ready_probe_host(host), port), timeout=0.5):
            return True
    except OSError:
        return False
...
def start(
    ...
    if state:
        ...
        if _is_process_running(pid):
            ...
            console.print(f"[yellow][WARN][/yellow] dataclaw 已在运行: pid={pid}, url=http://{existing_host}:{existing_port}")
            raise typer.Exit(1)
        _remove_state(pid_path)
        console.print("[yellow][WARN][/yellow] 检测到过期状态文件，已自动清理")

    if _is_port_accepting(host, port):
        console.print(f"[red][ERROR][/red] 端口已被占用: {host}:{port}")
        console.print("[yellow]请先停止占用该端口的进程，或换一个 --port[/yellow]")
        raise typer.Exit(1)
    ...
    if ready:
        console.print(f"[green][OK][/green] dataclaw 已启动: pid={process.pid}")
        console.print(f"[green][OK][/green] WebUI 地址: http://{host}:{port}")
        console.print(f"[green][OK][/green] 日志文件: {log_path}")
        return

    code = process.poll()
    if code is not None:
        _remove_state(pid_path)
        console.print(f"[red][ERROR][/red] dataclaw 启动失败，进程已退出 (code={code})")
        console.print(f"[yellow]日志文件[/yellow]: {log_path}")
        raise typer.Exit(1)

    console.print(f"[yellow][WARN][/yellow] 服务已拉起但未在 {ready_timeout:.1f}s 内确认就绪")
...
def status(
    ...
    state = _read_state(pid_path)
    if not state:
        console.print("[yellow][STOP][/yellow] dataclaw 状态: stopped")
        return
    ...
    if _is_process_running(pid):
        ...
        console.print("[green][RUN][/green] dataclaw 状态: running")
        console.print(f"[green]pid[/green]: {pid}")
        console.print(f"[green]url[/green]: http://{host}:{port}")
        return

    _remove_state(pid_path)
    console.print("[yellow][STOP][/yellow] dataclaw 状态: stopped (已清理过期状态文件)")
...
def stop(
    ...
    state = _read_state(pid_path)
    if not state:
        console.print("[yellow][WARN][/yellow] dataclaw 未运行")
        return

    pid = int(state.get("pid", 0))
    if not _is_process_running(pid):
        _remove_state(pid_path)
        console.print("[yellow][WARN][/yellow] dataclaw 进程不存在，已清理状态文件")
        return

    stopped = _stop_pid(pid, timeout)
    if stopped:
        _remove_state(pid_path)
        console.print(f"[green][OK][/green] dataclaw 已停止: pid={pid}")
        return

    console.print(f"[red][ERROR][/red] dataclaw 停止失败: pid={pid}")
    raise typer.Exit(1)
