...
def _task_to_with_history(task: A2ATask, history_length: Optional[int] = None, db: Optional[Session] = None) -> A2ATaskWithHistorySchema:
    if db is None:
        db = SessionLocal()
...
def _task_to_with_messages(task: A2ATask, db: Optional[Session] = None) -> A2ATaskWithMessagesSchema:
    if db is None:
        db = SessionLocal()
...
async def send_message(
    ...
    part_data = (message.parts[0].data if message.parts and message.parts[0].data else {}) or {}
    project_id = request.project_id or part_data.get("project_id")
    if not project_id:
    ...
    agent = None
    route_mode = request.route_mode or part_data.get("route_mode", "auto")
    remote_agent_id_param = request.remote_agent_id or part_data.get("remote_agent_id")
    ...
    idempotency_key = request.idempotency_key or part_data.get("idempotency_key")
    metadata = request.metadata or part_data.get("metadata", {})
...
@router.get(f"{A2A_API_PREFIX}/tasks/{{task_id}}")
def get_task(
    task_id: str,
    response: Response,
    ...
    return _task_to_with_history(task, history_length=historyLength, db=db)


@router.get(f"{A2A_API_PREFIX}/tasks")
def list_tasks(
    response: Response,
    ...
    status: Optional[SchemaTaskState] = Query(default=None, description="Filter by task status (A2A protocol)"),
    ...
    if state and state != "all":
        try:
            query = query.filter(A2ATask.state == A2ATaskState(state))
        except ValueError:
            pass
    elif status:
        query = query.filter(A2ATask.state == A2ATaskState(status.value))

    effective_limit = max(limit, pageSize)
    tasks = query.order_by(A2ATask.created_at.desc()).offset(0).limit(effective_limit).all()

    task_schemas = [_task_to_schema(item) for item in tasks]
    return [t.model_dump(mode='json') for t in task_schemas]


@router.post(f"{A2A_API_PREFIX}/tasks/{{task_id}}:cancel")
@router.post(f"{A2A_API_PREFIX}/tasks/{{task_id}}/cancel")
...
@router.get(f"{A2A_API_PREFIX}/tasks/{{task_id}}:subscribe")
@router.get(f"{A2A_API_PREFIX}/tasks/{{task_id}}/subscribe")
...
@router.post(f"{A2A_API_PREFIX}/tasks/{{task_id}}/pushNotificationConfigs", response_model=PushNotificationConfig, status_code=status.HTTP_201_CREATED)
...
@router.get(f"{A2A_API_PREFIX}/tasks/{{task_id}}/pushNotificationConfigs", response_model=List[PushNotificationConfig])
...
@router.delete(f"{A2A_API_PREFIX}/tasks/{{task_id}}/pushNotificationConfigs/{{config_id}}")
